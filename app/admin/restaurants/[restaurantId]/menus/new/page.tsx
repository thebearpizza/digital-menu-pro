...
export default function NewMenuPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.name.trim()) {
      setError('Il nome del menu è obbligatorio')
      return
    }
    setLoading(true)
    setError('')
    ...
    if (result.error) {
      ...
      router.push(`/admin/restaurants/${restaurantId}?created_menu=true`)
    }
  }
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-6 space-y-4">
          ...
          <p className="text-xs text-slate-400">
            Il banner viene mostrato nella landing pubblica quando il cliente scansiona il QR.
          </p>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">
              URL {form.banner_type === 'video' ? 'video' : 'immagine'}
            </label>
            <input
              type="url"
              name="banner_url"
            ...
            <p className="text-xs text-slate-400 mt-1">
              Puoi aggiungere o modificare il banner anche in seguito.
            </p>
          </div>
