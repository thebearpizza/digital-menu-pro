...
export default function MoveDishModal({
  restaurantId, fromMenuId, dishId, dishName, menus, onMoved, onClose,
}: Props) {
  ...
  async function handleMove() {
    ...
    try {
      await moveDishToMenu(restaurantId, fromMenuId, dishId, target)
      onMoved(dishId)
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white border border-gray-200 shadow-xl w-full max-w-sm z-10 p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-gray-900">Sposta piatto</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-700 text-xl leading-none">&times;</button>
        </div>

        <p className="text-xs text-gray-500 mb-4">
          Sposta <strong>{dishName}</strong> in un altro menu. Il piatto verrà rimosso
          da questo menu.
        </p>
