...
function GradientBackground() {
  ...
  useEffect(() => {
    ...
    scene.add(mesh)
    return () => { scene.remove(mesh); skyGeo.dispose(); skyMat.dispose() }
  }, [scene])
...
export function Experience() {
  return (
    <>
      ...
      <OrbitControls enablePan={false} enableZoom={false} minPolarAngle={Math.PI / 2.4} maxPolarAngle={Math.PI / 1.9} />
      <Environment preset='studio' />
      <directionalLight position={[2, 5, 2]} intensity={2.2} castShadow shadow-mapSize-width={2048} shadow-mapSize-height={2048} shadow-bias={-0.0001} />
