...
export default async function PublicMenuPage({
  ...
  const [{ data: rawMenus }, { data: banners }, { data: info }] = await Promise.all([
    supabase
      .from('menus')
      .select('id, name, sort_order, category_order, schedule_enabled, schedule_from, schedule_until, translations, menu_type, text_content')
  ...
  const menus = await Promise.all(
    visibleMenus.map(async menu => {
      ...
      return {
        ...
        menu_type:    ((menu as any).menu_type ?? 'dishes') as 'dishes' | 'text',
        text_content: (menu as any).text_content ?? null,
        dishes: sorted.map(d => ({
          id:              d.id as string,
